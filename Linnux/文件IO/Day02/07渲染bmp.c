...
int main(int argc, char const *argv[])
{
    ...
    // // bmp_buf 每三个字节写入到  buf 中
    // for (int i = 0; i < 800 * 480; i++)
    // {
    //     // 小字节序
    //     unsigned char blue = bmp_buf[i * 3 + 0];
    //     unsigned char green = bmp_buf[i * 3 + 1];
    //     unsigned char red = bmp_buf[i * 3 + 2];
    //     buf[i] = 0x0 | blue | green << 8 | red << 16;
    // }
    ...
    for (int y = 0; y < 480; y++)
    {
        for (int x = 0; x < 800; x++)
        {
            ...
            buf[y][x] = (0xFF << 24) | (red << 16) | (green << 8) | blue;
        }
    }
